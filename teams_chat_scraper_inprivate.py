...
class TeamsCollector:
    ...
    def save_chat_data(self, chat_name, messages):
        ...
        # Sanitize filenames to ensure compatibility with non-ASCII characters
        def sanitize_filename(name):
            # Normalize Unicode characters to ASCII (e.g., replace unsupported characters with safe alternatives)
        ...
        def clean_message(msg):
            ...
            def clean_extracted_content(content, author, attachments, images):
                content = re.sub(r"Â|\s+", " ", content)  # Remove non-breaking spaces and extra newlines
                content = re.sub(rf"\b{re.escape(author)}\b", "", content, flags=re.IGNORECASE)  # Remove author's name
                for attachment in attachments:
                    content = content.replace(attachment, "")  # Remove attachment names
                for image in images:
                    content = content.replace(image, "")  # Remove image names
                content = re.sub(r"\bTranslate\b", "", content, flags=re.IGNORECASE) # Remove "Translate" button or similar artifacts
                return content.strip()
    
            attachments = [att["name"] for att in msg.get("attachments", [])]
            images = [os.path.basename(img["local_path"]) for img in msg.get("images", []) if img["download_status"] == "success"]
            raw_content = extract_content_after_timestamp(msg.get("content", ""), msg.get("timestamp", ""))
            cleaned_content = clean_extracted_content(raw_content, msg.get("author", ""), attachments, images)
    
            msg["content"] = cleaned_content
            msg["attachments"] = ", ".join(attachments) if attachments else ""
            msg["images"] = ", ".join(images) if images else ""
            return msg
    
        # Apply cleanup to all messages
        messages = [clean_message(msg) for msg in messages]
    
        # Sanitize chat name for filenames
        safe_name = sanitize_filename(chat_name)
    
        # Save cleaned chat as JSON file
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        json_file = os.path.join(self.output_dir, f"{safe_name}_{timestamp}.json")
        with open(json_file, 'w', encoding='utf-8') as f:
            json.dump(messages, f, indent=2, ensure_ascii=False)  # Ensure Chinese characters are preserved
    
        # Save cleaned chat as CSV file
        csv_file = os.path.join(self.output_dir, f"{safe_name}_{timestamp}.csv")
        with open(csv_file, 'w', newline='', encoding='utf-8-sig') as f:  # Use UTF-8 with BOM for better compatibility
            fieldnames = ['message_id', 'author', 'timestamp', 'content', 'images', 'attachments']
        ...
        if self.download_images:
            image_count = sum(len(msg.get("images", "").split(", ")) for msg in messages)
